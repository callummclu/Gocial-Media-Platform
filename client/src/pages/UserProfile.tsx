...
function UserProfile(props:any){
    ...
    const checkUserNotSelfOrFriend = () => {
        let SameUser = (loggedInUserData?.username) == username
    ...
    const addUser = () => {
        fetch(`${process.env.REACT_APP_BACKEND_URI}/user/invitation/${loggedInUserData?.username}/${username}/${localStorage.getItem("gocial_auth_token")}`,{
            method:"POST"
        }).then(async (res:any) => {
            let res_json = await res.json()
    ...
    const acceptRequest = (req_user:string) => {
        fetch(`${process.env.REACT_APP_BACKEND_URI}/user/friends/${loggedInUserData?.username}/${req_user}/${localStorage.getItem("gocial_auth_token")}/accept`,{
            method:"POST"
        }).then(async (res:any) => {
            let res_json = await res.json()
            ...
            <div style={{display:"flex",gap:"30px"}}>
                ...
                <Card style={{width:"30%",height:"calc(100vh - 110px)", position:"sticky",top:"50px"}} withBorder >
                    ...
                    {(checkUserIsSelf() 
                        ...
                        <FriendsContainer>
                            {loggedInUserData?.received_invitations.map((e:any)=><Card className="user" p="sm" radius="md" key={e}><Group><Avatar radius="xl" /><Title onClick={()=>redirectToUser(e)} order={5}>{e}</Title><Button onClick={()=>acceptRequest(e)}>Accept</Button></Group></Card>)}
                        </FriendsContainer></>}
                </Card>
